...
class CreateOrdersTable extends Migration {
	...
	public function up()
	{
		Schema::create('orders', function(Blueprint $table)
		{
			...
			$table->bigInteger('cars_id')->index('fk_orders_cars1_idx')->comment('차량정보테이블 시퀀스');
			...
			$table->integer('status_cd')->comment('주문상태 코드,codes.order_status');
			$table->timestamps();
			$table->dateTime('diagnose_at')->nullable()->comment('진단일,정비사에게할당된 날짜');
			$table->dateTime('diagnosed_at')->nullable()->comment('진단완료일');
