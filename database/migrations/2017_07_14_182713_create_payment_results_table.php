...
class CreatePaymentResultsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('payment_results', function (Blueprint $table) {
            ...
            $table->string("tid", 30)->comment("거래번호");
            $table->string("stateCd")->comment("승인취소구분\n0:승인, 1:전취소, 2:후취소 3:가상계 좌발급 4:가상계좌입금기간만료");
            $table->string("authDate")->nullable()->comment("승인일자");
            $table->string("authCode", 255)->nullable()->comment("승인번호");
            $table->string("fnCd", 10)->nullable()->comment("결제카드사/은행, 코드값임");
            $table->string("fnName", 30)->nullable()->comment("결제카드사명/은 행명");
            $table->string("resultCd", 30)->comment("결제결과코드");
            $table->string("resultMsg", 255)->comment("결제결과메시지");
            $table->string("cardQuota", 10)->nullable()->comment("할부개월수");
            $table->string("cardNo", 30)->nullable()->comment("카드번호");
            ...
            $table->string("ediDate", 600)->comment("암호화된 시간. 복호화에 사용");
