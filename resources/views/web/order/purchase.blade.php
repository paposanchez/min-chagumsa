...
@section( 'content' )
    ...
    <div id='sub_wrap'>
        ...
        <div class='join_wrap'>
            ...
            <input name="goodsName" id="goodsName" type="hidden"
                   value="{{ $order->datekey }}-{{ $order->car_number }}">{{-- 상품명 --}}
            <input name="buyerName" id="buyerName" type="hidden" value="{{ $order->orderer_name }}">{{-- 구매자명 --}}
            <input name="buyerTel" id="buyerTel" type="hidden"
                   value="{!! str_replace("-", "", $order->orderer_mobile) !!}">{{-- 구매자연락처 --}}
            <input name="buyerEmail" id="buyerEmail" type="hidden"
                   value="{{ \Illuminate\Support\Facades\Auth::user()->email }}">{{-- 구매자메일주소 --}}
            ...
            <div class='item_info_box' id="item_info_box">
                ...
                <div class='ipt_line'>
                    ...
                    <div class="ipt_guide2 span_warning"> ※ 차량등록증에 등록된 배기량에 따라 정확히 선택해 주세요.<br> &nbsp;&nbsp;&nbsp;&nbsp;배기량
                        선택이 잘못된 경우, 차량 입고 시 재주문 처리가 될 수 있습니다.
                    </div>
                    <div class="br10"></div>
                    @foreach($items as $item)
                        <label>
                            <input type='radio' class='psk item_choice' name='item_choice' value="{{ $item->id }}"
                                   autocomplete="off"
                                   data-product_name="{{ $order->datekey }}-{{ $order->car_number }}/{{ $item->name }}({!! $item->car_sort == 'N'? '국산차': '수입차' !!})">
                            <span class='lbl'> {{ $item->name }}<span
                                        style="color: #a3cd16;"> ({!! $item->car_sort == 'N'? '국산차': '수입차' !!}
                                    )</span></span>
                        </label>&nbsp;&nbsp;&nbsp;&nbsp;
                    @endforeach
            ...
            <div class='ipt_line wid45'>
                <button class='btns btns_blue wid45' style='display:inline-block;' type="button" id="payment-back">이전
                </button>&nbsp;&nbsp;
                <button type="button" class='btns btns_green wid45' style='display:inline-block;' id="payment-process">
                    결제진행
                </button>
            </div>
            {!! Form::close() !!}
