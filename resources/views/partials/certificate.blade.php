                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_exterior_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_exterior_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_exterior_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_flooded_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_flooded_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_flooded_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_consumption_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_consumption_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_consumption_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_broken_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_broken_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_broken_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_power_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_power_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_power_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_electronic_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_electronic_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_electronic_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_interior_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_interior_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_interior_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_exteriortest_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_exteriortest_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_exteriortest_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_plugin_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_plugin_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_plugin_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_engine_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_engine_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_engine_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_steering_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_steering_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_steering_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_tire_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_tire_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_tire_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_accident_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_accident_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_accident_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_interiortest_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_interiortest_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_interiortest_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_driving_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_driving_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_driving_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_transmission_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_transmission_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_transmission_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
                                ...
                                <div class="btn-group" data-toggle="buttons">
                                        @foreach($certificate_states as $key=>$val )
                                                <label class="btn btn-default {{ $order->certificates->performance_braking_cd == $key ? 'active' : '' }}">
                                                        {{ Form::radio('performance_braking_cd', $key, \App\Helpers\Helper::isCheckd($key, $order->certificates->performance_braking_cd)) }} {{ $val }}
                                                </label>
                                        @endforeach
                                </div>
